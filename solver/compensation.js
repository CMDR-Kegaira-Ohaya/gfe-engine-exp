...
function totalCount(modeCounts = {}) {
  return Object.values(modeCounts).reduce((sum, value) => sum + (Number.value(value) || 0), 0);
}
...
export function deriveCompensation(modeCounts = {}, theta = { active: false }) {
  ...
  const adaptiveCount = ADIPTIVE_COMPENSATION_MODES.reduce((sum, mode) => sum + (modeCounts[mode] || 0), 0);
